...
class Parser(Tap):
    ...
    def generate_exp_name(self, args):
        ...
        if callable(exp_name):
            ...
            use_tree = getattr(args, "use_tree", False)

            multi_task = getattr(args, "multi_task", False)
            tree_lambda = getattr(args, "tree_lambda", None)
            one_minus_alpha = getattr(args, "one_minus_alpha", None)

            if use_tree:
                exp_name_string += f'_multi{multi_task}_tree_lambda{tree_lambda}_th{one_minus_alpha}'
            else:
                exp_name_string += f'_multi{multi_task}_vanilla'
